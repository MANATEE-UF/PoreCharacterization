function out = imover(in, mask, color)
...
% color value.
out_red(mask)   = round(color(1)*255,0)+out_red(mask);
out_green(mask) = round(color(2)*255,0)+out_green(mask);
out_blue(mask)  = round(color(3)*255,0)+out_blue(mask);
